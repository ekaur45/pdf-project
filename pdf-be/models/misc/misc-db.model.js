...
Misc.updateHotel = async data => {
    const result =  await mysqlExecute('update hotel set `name`=?,location=?,address=?,price=?,priceCurrency=? where id=?', [data.name, data.destination,data.address,Number(data.price),data.priceCurrency, data.id], false);
